function progSliderCalcPos(el, event) {
    var handle = el.find('.progSlider-handle');
    var vertical = el.attr('data-progSlider-vertical') === 'true' ? true : false;
    var perc = vertical ? 1 - ( (event.pageY - el.offset().top) / el.height()) :  (event.pageX - el.offset().left) / el.width();
    var val=parseInt(el.attr('data-progSlider-max')*perc, 10);  
    val <= el.attr('data-progSlider-max') && val >= el.attr('data-progSlider-min') && el.children('.progSlider-progress').css(vertical ? 'height' : 'width', perc*100 + '%') && el.trigger('onSlide', [val]);
}

$('html').on('mouseup', function (event) {
    progSlider=$('[data-progSlider-drag=true]');  
    progSlider.length>0 && progSlider.attr('data-progSlider-drag', false).trigger('onSet', [progSlider.getProgSlider()]);
});
...
$.fn.extend({
    progSlider: function (options) {
        ...
        return this.each(function () {        
            ...
            var isInit=progSlider.attr('data-progSlider')==='true'?true:false;
            !isInit && progSlider.html('').append("<div class='progSlider-progress'><div class='progSlider-handle'></div></div>");
            ...
            config.onSlide && progSlider.on('onSlide', function (event, val) {
                config.onSlide(val);
                console.log(progSlider);
                progSlider.trigger('onChange', [val]);
            });
            config.onSet && progSlider.on('onSet', function (event, val) {
                config.onSet(val);
                console.log(progSlider);
                progSlider.trigger('onChange', [val]);
            });       
            config.onChange && progSlider.on('onChange', function (event, val) {
                console.log(990);
                config.onChange(val);
            });            
            !isInit && progSlider.on('click', function (event) {
                progSliderCalcPos($(this), event);
            });
            !isInit && progSlider.on('mousedown', function (event) {
    ...
    setProgSlider: function (val, trigger) {    
        ...
        progSliders.each(function (index, progSlider) {
            ...
            if (progSlider.attr('data-progSlider') === 'true') {
                ...
                trigger === true && progSlider.trigger('onSet', [val]);
            }
        });
    },
    getProgSlider: function (val) {
        var progSliders = $(this);
        var values=[];
        progSliders.each(function (index, progSlider) {
            progSlider = $(progSlider);
            var vertical = (progSlider.attr('data-progSlider-vertical') === 'true') ? true : false;
            var val = vertical ? progSlider.children('.progSlider-progress').height() / progSlider.height() : progSlider.children('.progSlider-progress').width() / progSlider.width();
